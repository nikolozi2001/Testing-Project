...
// ------------------------------------------------------------------------------------------------------------------
// ეს არის ახალი კოდი რომელიც აჩვენებს ლაბელს
...
if (Array.isArray(data["page1EditGrid"])) {
  data["page1EditGrid"].forEach((rowData, rowIndex) => {
        ...
        if (
          ...
          if (isEmpty(value)) {
            ...
            console.log(
              `სტრიქონი ${
                rowIndex + 1
              }-ში ველი "${firstEmptyLabel}" არ არის შევსებული`
            );
          }
        }
      }
    ...
    if (!allFilled) {
      valid = `სტრიქონი ${
        rowIndex + 1
      }-ში ველი "${firstEmptyLabel}" ცარიელია. ყვითელ ფანქარზე დაჭერით შეავსეთ ფილიალის მონაცემები`;
    }
  });
}
